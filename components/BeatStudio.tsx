...
import {
  StudioWorkerClient,
  isSupersededWorkerRequest,
} from "../lib/workers/studio-worker-client";
import { SampleAccurateAudioEngine, type PlaybackMode } from "../lib/music/audio-transport";
import { TICKS_PER_STEP, buildCanonicalTimeline } from "../lib/music/canonical-timeline";
import { usePlayheadController } from "../lib/music/usePlayheadController";
import { deriveSeed } from "../lib/core/seeds/manager";
import {
...
function createUnlockedSeed(): number {
  if (typeof crypto === "undefined" || !crypto.getRandomValues) {
    throw new Error("Web Crypto indisponível para criar uma seed desbloqueada");
  }
...
export default function BeatStudio() {
  ...
  const syncVisiblePlaybackSection = useCallback((sectionIndex: number) => {
    ...
    setMelodyLayers((previous) => previous.map((layer) => ({
      ...layer,
      result: block.melodyResults.find((entry) => entry.layerId === layer.id)?.result ?? null,
    })));
  }, []);

  // The RAF playhead reads the canonical transport clock and keeps the
  // visible section synchronized with Intro/Verse/Drop during full playback.
  ...
  const startPlayback = useCallback((mode: PlaybackMode, blockIndex?: number) => {
    ...
    if (mode === "all" && (selectedBlock || s.arrangementBlocks.length > 0)) {
      ...
      const prepared = audioEngineRef.current.getPreparedTimelineSummary();
      if (prepared.timelineHash !== expectedTimeline.timelineHash) {
        audioEngineRef.current.stop();
        setPlaybackMode(null);
    ...
    async (presetId: string, customConfig?: ArtistPresetConfig) => {
      ...
      if (workerClientRef.current) {
        ...
        try {
          ...
          if (allData.blocks.length > 0) {
            setArrangementBlocks(allData.blocks);
            setGenerationIdentity(allData.identity);
  ...
  const patchBlockById = useCallback((blockId: string, patch: Partial<ArrangementBlockData>) => {
    setArrangementBlocks((prev) => prev.map((block) =>
  ...
  const patchActiveBlock = useCallback((patch: Partial<ArrangementBlockData>) => {
    const state = stateRef.current;
    const blockId = state.arrangementBlocks[state.currentBlockIndex]?.id;
    if (blockId) patchBlockById(blockId, patch);
  }, [patchBlockById]);

  const publishMelodyResult = useCallback((
    blockId: string,
    layerId: string,
    result: NonNullable<MelodyLayer["result"]>,
  ) => {
    setArrangementBlocks((prev) => prev.map((block) => {
      if (block.id !== blockId) return block;
      const retained = block.melodyResults.filter((entry) => entry.layerId !== layerId);
      return { ...block, melodyResults: [...retained, { layerId, result }] };
    }));

    const state = stateRef.current;
    if (state.arrangementBlocks[state.currentBlockIndex]?.id === blockId) {
      setMelodyLayers((prev) => prev.map((layer) =>
    ...
    async (synthType: MelodySynthType) => {
      ...
      if (workerClientRef.current) {
        ...
        try {
          ...
          publishMelodyResult(targetBlock.id, newLayerId, result);
        } catch (err: unknown) {
          if (!isSupersededWorkerRequest(err) && mutationEpoch === mutationEpochRef.current) {
            setError(err instanceof Error ? err.message : "Erro ao gerar nova camada.");
          }
        } finally {
          if (
            selectiveEpochRef.current[operationKey] === requestEpoch
    ...
    setMelodyLayers((prev) => {
      const next = prev.map((layer) => {
        ...
        const degreeIndex = deterministicIndex(
          stateRef.current.compositionSeed,
          stateRef.current.variationIndex,
          `manual:melody:${activeBlockId}:${layerId}:${stepIdx}`,
  ...
  const generateMelodyLayer = useCallback(
    async (layerId: string, customSeed?: number) => {
      ...
      try {
        ...
        publishMelodyResult(targetBlock.id, layerId, result);
      } catch (err: unknown) {
        if (
          !isSupersededWorkerRequest(err)
          && selectiveEpochRef.current[operationKey] === requestEpoch
    ...
    async (engine: "bass" | "drums", customSeed?: number) => {
      ...
      try {
        if (engine === "bass") {
          ...
          patchBlockById(targetBlock.id, { bass: bassData });
          const state = stateRef.current;
          ...
          patchBlockById(targetBlock.id, { drums: drumsData });
          const state = stateRef.current;
          if (state.arrangementBlocks[state.currentBlockIndex]?.id === targetBlock.id) {
            setDrums(drumsData);
          }
  ...
  const toggleBassStep = (stepIdx: number) => {
    ...
    const degreeIndex = deterministicIndex(
      stateRef.current.compositionSeed,
      stateRef.current.variationIndex,
      `manual:bass:${activeBlockId}:${stepIdx}`,
  ...
  const handleExportWav = async () => {
    ...
    try {
      ...
      const result = await workerClientRef.current.exportWav({
        ...
        bassDrive,
        drumKit,
        trackSettings: stateRef.current.trackSettings,
  ...
  useEffect(() => {
    ...
    if (restored) {
      ...
      const restoredBlockIndex = Math.max(
        0,
        restored.blocks.findIndex((block) => block.id === restored.currentBlockId),
      );
      const restoredBlock = restored.blocks[restoredBlockIndex] ?? restored.blocks[0];

      setBpm(controls.bpm);
      setArtistPreset(controls.artistPreset as ArtistPresetId);
      setBpmInput(String(controls.bpm));
      setKey(controls.key);
      setGlobalScale(controls.globalScale);
      setComplexity(controls.complexity);
      setBassStyle(controls.bassStyle);
      setBassOctave(controls.bassOctave);
      setBassDrive(controls.bassDrive);
      setDrumStyle(controls.drumStyle);
      setDrumPattern(controls.drumPattern);
      setDrumKit(controls.drumKit);
      setDrumSwing(controls.swing);
      setDrumRollDensity(controls.rollDensity);
      setDrumHumanize(controls.humanize);
      setMuteBass(controls.muteBass);
      setMuteDrums(controls.muteDrums);
      setIsLooping(controls.isLooping);
      setCompositionSeed(restored.seed);
      setSeedLocked(restored.seedLocked);
      setVariationIndex(restored.variationIndex);
      setGenerationIdentity(restored.identity);
      setCompositionTimeline(restored.timeline);
      setCompositionId(restored.compositionId);
      setTrackSettings(restored.trackSettings);
      setArrangementBlocks(restored.blocks);
      setCurrentBlockIndex(restoredBlockIndex);
      setBass(restoredBlock.bass);
      setDrums(restoredBlock.drums);
      setMelodyLayers(restored.melodyLayers.map((layer) => ({
        ...layer,
        result: restoredBlock.melodyResults.find((entry) => entry.layerId === layer.id)?.result ?? null,
      })));
      ...
      workerClient.generateAll({
        ...
        if (
          ...
          setError(err instanceof Error ? err.message : "Erro ao gerar composição inicial.");
        }
      }).finally(() => {
        if (
          requestEpoch === generationEpochRef.current
  ...
  useEffect(() => {
    ...
    const currentBlock = arrangementBlocks[currentBlockIndex] ?? arrangementBlocks[0];
    ...
    const resolvedTimeline: CompositionTimelineMetadata = {
      ppq: 960,
      totalSteps: canonicalTimeline.arrangementEndTick / TICKS_PER_STEP,
      endTick: canonicalTimeline.arrangementEndTick,
      arrangementEndTick: canonicalTimeline.arrangementEndTick,
      hash: canonicalTimeline.timelineHash,
    };
    if (
      compositionTimeline.hash !== resolvedTimeline.hash
      || compositionTimeline.endTick !== resolvedTimeline.endTick
    ) {
      setCompositionTimeline(resolvedTimeline);
    }
    const snapshot: CompositionSessionSnapshot = {
      snapshotVersion: COMPOSITION_SNAPSHOT_VERSION,
      seed: compositionSeed,
      seedLocked,
      variationIndex,
      compositionId: generationIdentity.generationId,
    ...
    try {
      localStorage.setItem(COMPOSITION_STORAGE_KEY, serializeCompositionSnapshot(snapshot));
    } catch {
      setError("Não foi possível persistir a composição neste navegador.");
    }
  }, [
    generationIdentity,
    compositionTimeline,
